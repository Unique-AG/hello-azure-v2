...
echo "=========================================="
echo ""
echo "⚠️  Role assignments require complex import format: <scope>|<principal_id>|<role_definition_id>"
echo "    Getting these values programmatically is complex."
echo ""
echo "To import role assignments, you need:"
echo "  1. Scope: The resource ID where the role is assigned (e.g., Key Vault ID, Resource Group ID, AKS Cluster ID)"
echo "  2. Principal ID: The object ID of the identity/user/service principal"
echo "  3. Role Definition ID: The GUID of the role definition (not the name)"
echo ""
echo "Example commands to get role assignment details:"
echo "  # Get role definition ID by name:"
echo "  az role definition list --name 'Key Vault Secrets User' --query '[0].id' -o tsv"
echo ""
echo "  # List role assignments on a resource:"
echo "  az role assignment list --scope <resource-id> --query '[].{principalId:principalId,roleDefinitionId:roleDefinitionId}' -o table"
echo ""
echo "  # Import format:"
echo "  terraform import -var-file=\"${VAR_CONFIG}\" -var-file=\"${VAR_PARAMS}\" \\"
echo "    azurerm_role_assignment.<name> \\"
echo "    '<scope>|<principal_id>|<role_definition_id>'"
echo ""
echo "⚠️  Role assignments will be imported manually or skipped for now."
echo "    They may show as 'to be created' in terraform plan initially."
echo ""
